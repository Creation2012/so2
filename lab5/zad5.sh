...
# spróbuj zrobić bez podwójnego pipe na cut może jakieś tr? ja bym robił sedem
cat "${FILE}" | grep -e "syslog*" | tr -s ' ' | cut --delimiter=' ' --fields=2 | cut --delimiter='/' --fields=1 | sort | uniq
